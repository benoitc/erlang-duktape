%%%-------------------------------------------------------------------
%%% @doc Benchmark runner for duktape
%%%
...
%%%
%%% Usage:
%%%   rebar3 as bench eunit --module=duktape_bench
%%%
%%% Or programmatically:
%%%   duktape_bench:run_all().
%%%   duktape_bench:run(eval_simple).
%%%
%%% @end
%%%-------------------------------------------------------------------
-module(duktape_bench).
...
bench_eval_simple(_Opts) ->
    ...
    try
        {ok, _} = duktape:eval(Ctx, <<"1 + 2 * 3 - 4 / 2">>),
        1
    after
...
bench_eval_bindings_small(_Opts) ->
    ...
    try
        {ok, _} = duktape:eval(Ctx, <<"a + b + c + d + e">>, Bindings),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_eval_bindings_large(_Opts) ->
    ...
    try
        {ok, _} = duktape:eval(Ctx, Code, Bindings),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_call_with_args(_Opts) ->
    {ok, Ctx} = duktape:new_context(),
    {ok, _} = duktape:eval(Ctx, <<"function sum5(a,b,c,d,e) { return a+b+c+d+e; }">>),
    try
        {ok, _} = duktape:call(Ctx, sum5, [1, 2, 3, 4, 5]),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_call_many_args(_Opts) ->
    ...
    {ok, _} = duktape:eval(Ctx, Code),
    try
        {ok, _} = duktape:call(Ctx, sum20, Args),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_type_convert_simple(_Opts) ->
    {ok, Ctx} = duktape:new_context(),
    try
        {ok, _} = duktape:eval(Ctx, <<"x + ' ' + y">>, #{x => 12345, y => <<"hello">>}),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_type_convert_array(_Opts) ->
    ...
    try
        {ok, _} = duktape:eval(Ctx, <<"arr.length">>, #{arr => Array}),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_type_convert_nested(_Opts) ->
    ...
    try
        {ok, _} = duktape:eval(Ctx, <<"JSON.stringify(data)">>, #{data => NestedData}),
        1
    after
        duktape:destroy_context(Ctx)
    end.
...
bench_module_require_cached(_Opts) ->
    ...
    {ok, _} = duktape:require(Ctx, <<"math">>),
    try
        %% Measure cached requires
        {ok, _} = duktape:eval(Ctx, <<"require('math').add(1, 2)">>),
        1
    after
        duktape:destroy_context(Ctx)
    end.
