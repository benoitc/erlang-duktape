...
ok = duktape:register_function(Ctx, <<"double">>, fun(X) -> X * 2 end).

%% Call from JavaScript
{ok, 10} = duktape:eval(Ctx, <<"double(5)">>).
```
...
```erlang
ok = duktape:register_function(Ctx, <<"add">>, fun(A, B) -> A + B end).
```

### Module Functions

```erlang
ok = duktape:register_function(Ctx, <<"upcase">>, {string, to_upper}).
{ok, <<"HELLO">>} = duktape:eval(Ctx, <<"upcase('hello')">>).
```
...
```erlang
ok = duktape:register_function(Ctx, <<"process">>, fun(Data) ->
...
{ok, Result} = duktape:eval(Ctx, <<"process({name: 'Alice', age: 30})">>).
...
```erlang
ok = duktape:register_function(Ctx, <<"get_config">>, fun() ->
    #{
        host => <<"localhost">>,
        port => 8080,
        debug => true,
...
{ok, 20} = duktape:eval(Ctx, <<"double(double(5))">>).
{ok, 80} = duktape:eval(Ctx, <<"double(double(double(10)))">>).
```
...
```erlang
ok = duktape:register_function(Ctx, <<"divide">>, fun(A, B) ->
    case B of
...
{error, _} = duktape:eval(Ctx, <<"divide(10, 0)">>).
```

You can also throw specific error messages:

```erlang
ok = duktape:register_function(Ctx, <<"validate">>, fun(Age) ->
    if
...
run() ->
    ...
    ok = duktape:register_function(Ctx, <<"http_get">>, fun http_get/1),
    ok = duktape:register_function(Ctx, <<"base64_encode">>, {base64, encode}),
    ok = duktape:register_function(Ctx, <<"md5">>, fun(Data) ->
