...
This guide will help you get up and running with Duktape, an embedded JavaScript engine for Erlang.

## Installation

Add `duktape` to your `rebar.config` dependencies:
...
{ok, <<"Alice">>} = duktape:eval(Ctx, <<"user.name">>, #{
    user => #{name => <<"Alice">>, age => 30}
}).

%% Arrays
{ok, 6} = duktape:eval(Ctx, <<"nums.reduce(function(a,b){return a+b}, 0)">>, #{
    nums => [1, 2, 3]
}).
```
...
%% Call it
{ok, <<"Hello, World!">>} = duktape:call(Ctx, greet, [<<"World">>]).

%% Call without arguments
{ok, _} = duktape:eval(Ctx, <<"function now() { return Date.now(); }">>).
{ok, Timestamp} = duktape:call(Ctx, now).
```
...
%% Register a module
ok = duktape:register_module(Ctx, <<"math-utils">>, <<"
    exports.square = function(x) { return x * x; };
    exports.cube = function(x) { return x * x * x; };
">>).

%% Use the module
{ok, 25} = duktape:eval(Ctx, <<"require('math-utils').square(5)">>).
{ok, 27} = duktape:eval(Ctx, <<"require('math-utils').cube(3)">>).
```
