...
Monitor memory usage and trigger garbage collection for Duktape contexts.

## Overview

Each Duktape context tracks memory metrics via a custom allocator:
...
%% Do some work
{ok, _} = duktape:eval(Ctx, <<"var data = []; for(var i=0; i<1000; i++) data.push(i);">>).
...
{ok, #{
    ...
    free_count => 89,
    gc_runs => 2
}} = duktape:get_memory_stats(Ctx).
```

## Garbage Collection

Manually trigger Duktape's garbage collector:
...
{ok, Before} = duktape:get_memory_stats(Ctx).
...
{ok, After} = duktape:get_memory_stats(Ctx).
...
check_memory(Ctx) ->
    ...
    case maps:get(heap_bytes, Stats) of
        Bytes when Bytes > ?MEMORY_THRESHOLD ->
            ...
            ok = duktape:gc(Ctx),
            {ok, NewStats} = duktape:get_memory_stats(Ctx),
...
log_stats(Stats) ->
    logger:info("Duktape memory: ~p bytes, ~p allocs, ~p GC runs",
...
{error, invalid_context} = duktape:get_memory_stats(Ctx).
{error, invalid_context} = duktape:gc(Ctx).

%% Invalid context reference
{error, invalid_context} = duktape:get_memory_stats(not_a_context).
```
