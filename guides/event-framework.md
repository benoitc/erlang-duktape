...
{ok, <<"Hello, Alice!">>} = duktape:send(Ctx, greeting, #{name => <<"Alice">>}).
```
...
run() ->
    ...
    {ok, 5} = duktape:send(Ctx, increment, #{amount => 5}),
    {ok, 8} = duktape:send(Ctx, increment, #{amount => 3}),
    {ok, 8} = duktape:send(Ctx, get_counter, #{}),
