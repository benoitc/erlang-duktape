...
Duktape uses its native CBOR implementation for encoding and decoding.
...
{ok, Bin1} = duktape:cbor_encode(Ctx, 42).
{ok, Bin2} = duktape:cbor_encode(Ctx, <<"hello">>).
{ok, Bin3} = duktape:cbor_encode(Ctx, true).

%% Encode complex structures
{ok, Bin4} = duktape:cbor_encode(Ctx, #{
    name => <<"Alice">>,
    age => 30,
    tags => [<<"admin">>, <<"user">>]
}).
...
```erlang
{ok, 42} = duktape:cbor_decode(Ctx, Bin1).
{ok, <<"hello">>} = duktape:cbor_decode(Ctx, Bin2).
{ok, true} = duktape:cbor_decode(Ctx, Bin3).

{ok, #{
    <<"name">> := <<"Alice">>,
    <<"age">> := 30,
    <<"tags">> := [<<"admin">>, <<"user">>]
}} = duktape:cbor_decode(Ctx, Bin4).
```
...
%% Encode
{ok, Binary} = duktape:cbor_encode(Ctx, Original).

%% Binary is compact
byte_size(Binary).  %% Much smaller than JSON

%% Decode
{ok, Decoded} = duktape:cbor_decode(Ctx, Binary).
...
{ok, Binary} = duktape:cbor_encode(Ctx1, large_data_structure()).

%% Decode in another context
{ok, Ctx2} = duktape:new_context().
{ok, Data} = duktape:cbor_decode(Ctx2, Binary).
```
...
save_config(Ctx, Config, Filename) ->
    {ok, Binary} = duktape:cbor_encode(Ctx, Config),
    file:write_file(Filename, Binary).

load_config(Ctx, Filename) ->
    {ok, Binary} = file:read_file(Filename),
    duktape:cbor_decode(Ctx, Binary).
```
...
send_message(Ctx, Socket, Message) ->
    {ok, Binary} = duktape:cbor_encode(Ctx, Message),
...
{ok, _} = duktape:cbor_encode(Ctx, very_large_structure()),
{ok, _} = duktape:cbor_decode(Ctx, large_binary()).
```
...
```erlang
%% Invalid CBOR binary
{error, _} = duktape:cbor_decode(Ctx, <<"not valid cbor">>).

%% Invalid context
{error, invalid_context} = duktape:cbor_encode(destroyed_ctx, data).
```
